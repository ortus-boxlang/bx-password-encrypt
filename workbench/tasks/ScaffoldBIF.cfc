...
component {
	...
	function init(){
		...
		variables.util
			.addMapping( '/bifsDirectory',	"#variables.root#/src/main/java/ortus/boxlang/modules/encrypt/bifs" )
			.addMapping( '/bifTestsDirectory', "#variables.root#/src/test/java/ortus/boxlang/modules/encrypt/bifs" );
	...
	function run(
		...
		if( listLen( package, "." ) == 1 ){
			arguments.package = "ortus.boxlang.modules.encrypt.bifs." & package;
			...
			var bifTargetDirectory = expandPath( "/bifsDirectory" );
			if( arguments.withTests ){
				var testTargetDirectory = expandPath( "/bifTestsDirectory" );
			}
		}
